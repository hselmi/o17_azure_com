...
class PlanSaleOrder(models.Model):
    ...
    def process_contract(self):
        for order in self:
            custom_plan_orderlines = list(filter(lambda line: line.is_custom_plan and not line.is_processed, order.order_line))
            for line in custom_plan_orderlines:
                line.is_processed = True
                _logger.info(" -- -- - - - process contract executed in custom plan module ---")
                saas_users = None
                ...
                server_id = None
                if not default_data['is_multi_server']:
                    server_id = self.env['saas.server'].sudo().search([('state','=', 'confirm')]).filtered(lambda s: s.total_clients < s.max_clients)[:1]
                else:
                    priority_servers = line.odoo_version_id.default_saas_servers_ids.sorted(lambda s: s.priority)
                ...
                vals = dict(
                    ...
                    currency_id=order.partner_id.property_product_pricelist and order.partner_id.property_product_pricelist.currency_id and order.partner_id.property_product_pricelist.currency_id.id or False,
                    ...
                    free_users=default_data['free_users'],
                    ...
                    # server_id=server_id and server_id.id if not default_data['is_multi_server'] else False,
                    server_id=server_id and server_id.id,
                ...
                try:
                    ...
                    modules = self.env['saas.module'].sudo().search([('is_published', '=', True), ('odoo_version_id.code', '=', line.odoo_version_id.code or '17.0')])
                    extra_modules = []
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if line_id and order_line.exists() and order_line.id == res['line_id']:
            order_line.write({
