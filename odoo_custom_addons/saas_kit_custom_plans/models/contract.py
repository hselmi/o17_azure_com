...
class ContractCustomPlan(models.Model):
    ...
    update_saas_module_ids = fields.One2many(comodel_name="saas.module",inverse_name="contract_id", string="Related Modules Updated")
    ...
    @api.depends('plan_id', 'odoo_version_id')
    def get_code(self):
        for rec in self:
            rec.version_code = '17.0'
            if rec.odoo_version_id:
                rec.version_code = rec.odoo_version_id.code
            elif rec.plan_id.provide_odoo_version:
                # and rec.plan_id.plan_odoo_version.code
