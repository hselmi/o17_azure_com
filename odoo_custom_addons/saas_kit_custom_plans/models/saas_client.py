            ...
            if obj.saas_contract_id.is_custom_plan:
                try:
                    if obj.state == "inactive":
                        ...
                        response = client.main(obj.database_name, obj.containter_port, host_server, get_module_resource('odoo_saas_kit'), from_drop_db=True, version=obj.saas_contract_id.odoo_version_id.code or '17.0')
        ...
        for obj in self:
            if obj.saas_contract_id.is_custom_plan:
                if obj.state == "inactive" and obj.container_id:
                    host_server, db_server = obj.saas_contract_id.server_id.get_server_details()
                    _logger.info("HOST SERER %r   DB SERVER  %r"%(host_server,db_server))
                    response = client.main(obj.database_name, obj.containter_port, host_server, get_module_resource('odoo_saas_kit'), container_id=obj.container_id, db_server=db_server, from_drop_container=True, version=obj.saas_contract_id.odoo_version_id.code or '17.0')
